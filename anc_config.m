...
% 扬声器映射简化：只指定通道号（1~4）
cfg.spkChannels      = [1, 2, 3, 4];  % 逻辑扬声器编号到物理通道的映射（Spk1→Ch1, ..., Spk4→Ch4）
...
cfg.spkAmplitude      = [1.2, 1.2];            % 播放扫频信号时各扬声器的增益（>1 表示数字域放大，需注意不削波）
cfg.sweepF1           = 80;                    % 扫频信号起始频率（Hz）；避开无效低频（<60 Hz）
cfg.sweepF2           = 1200;                  % 扫频信号终止频率（Hz）；覆盖 ANC 主要工作带宽
cfg.repetitions       = 4;                     % 重复播放并录制次数；用于提高 SNR 和鲁棒性
cfg.timeFrameSamples  = 512;                   % 录音/播放缓冲区帧大小（必须是 2 的幂，匹配 ASIO 或 DirectSound 缓冲）
...
cfg.doAlignRepeats    = false;                 % 是否对多次重复测量进行时间对齐（false 保留真实延迟变化）
...
cfg.writeBlockPad     = true;                  % 是否在播放块前后填充静音（避免突发）

% 峰与漂移相关参数
cfg.enableLowFreqBoost    = true;              % 是否在扫频中增强低频能量（提升低频 SNR）
...
cfg.prePeakKeep           = 32;                   % 主峰前保留的样本数（用于因果性检查）
cfg.deconvPreDelayKeep    = 768;                 % 反卷积结果中强制保留的前导静音长度
cfg.deconvPeakThreshDB    = 3;                   % 峰值检测阈值（dB，相对于最大值）；越低越敏感
cfg.deconvMaxSearch       = 9000;                % 最大搜索延迟范围（样本）
cfg.deconvRegEps          = 1e-3;                % Tikhonov 正则化参数；抑制噪声放大（值越大越平滑）
cfg.deconvNoiseWin        = 1200;                % 用于估计噪声功率的窗口长度（样本）
cfg.envSmoothWin          = 10;                  % 包络平滑窗口长度（样本）
cfg.deconvCumEnergyFrac   = 0.05;                % 累积能量达到该比例时视为有效起点
cfg.deconvMinPeakFrac     = 0.005;               % 最小峰值能量占比（防虚假峰）
cfg.deconvSnrBodyRadius   = 96;                  % SNR 计算时主峰邻域半径（样本）
cfg.deconvFftCorrEnable   = true;                % 是否启用 FFT 域互相关辅助峰值定位

% sweepCfg参数（扫频信号生成）
cfg.padLeading        = 0.25;    % 扫频前导静音时间（秒）
cfg.padTrailing       = 0.25;    % 扫频尾随静音时间（秒）
cfg.amplitude         = 0.98;    % 扫频信号幅值（接近满幅但避免削波）
cfg.sweepDuration     = 16;      % 扫频持续时间（秒）；越长频率分辨率越高，低频能量越强
...
cfg.enableAdaptiveDelay   = true;     % 是否自适应调整次级路径延迟（应对温度/位置漂移）
